    ...
    const getCandidate= async () => {
      try {
        ...
        if (response?.data?.success) {
          ...
          const currUser = candidates.find(user => user._id === id);
          if (!currUser) {
            navigate('/candidate');
          } else {
            isMounted && setCandidate(currUser);
            setCandidate((prev) => {
              return { ...prev, primarySkills: currUser.primarySkills.toString() }
            })
            setCandidate((prev) => {
              return { ...prev, secondarySkills: currUser.secondarySkills.toString() }
            })
          }
        }
      } catch (error) {
