...
const Interview = () => {
  ...
  useEffect(() => {
    ...
    const getPanel = async () => {
      try {
        ...
        if (response?.data?.success) {
          isMounted && setPanel(response?.data?.data);
  ...
  }, [axiosInterview, setInterviews, navigate, location]);
  ...
  const getUser = async (id) => {
    try {
      ...
      if (response?.data?.success) {
        ...
        const currHr = panel.find((member) => member._id == currUser.hrId);
        setHr(currHr);
        console.log("tech", currUser.techId);

        const currTech = panel.find((user) => user._id == currUser.techId);
        console.log("curr", currTech);
        setTech(currTech);
        setUser(currUser);
        if (!currUser) {
          navigate("/user");
  ...
  const handleSearchChange = (e) => {
    ...
    const resultsArray = interviews.filter((interview) =>
      interview.name
        .toString()
        .toLowerCase()
  ...
  return (
    <>
      ...
      <Container
        maxWidth="lg"
        style={{ marginLeft: "7%", marginTop: "1%", marginBottom: "3%" }}
      >
                ...
                <TableRow key={interview._id}>
                  ...
                  >
                    {interview.status=== "selected"?
                      <Chip label={interview.status}color="success" size="small" />:interview.status
                       }

                    {/* {interview.status} */}
                  </TableCell>
                  <TableCell align="right">
                    {interview.techRating === -1
                      ? "No Ratings given"
                      : interview.techRating}
                  </TableCell>
                  <TableCell align="right">
                    {interview.techComments === " "
                      ? "No Comments"
                      : interview.techComments}
                  </TableCell>
                  <TableCell align="right">
                    {interview.hrRating === -1
                      ? "No Ratings given"
                      : interview.hrRating}
                  </TableCell>
                  <TableCell align="right">
                    {interview.hrComments === ""
                      ? "No Comments"
                      : interview.hrComments}
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <Tooltip title="View user Details">
                      <IconButton onClick={() => getUser(interview._id)}>
        ...
        <Box sx={style}>
          ...
          <CardContent>
            ...
            <Typography color="text.secondary">
              <span style={{ fontWeight: "bold" }}>Tech Member:</span>{" "}
              {tech.userName}
            </Typography>{" "}
            <Typography color="text.secondary">
              <span style={{ fontWeight: "bold" }}>Hr Member:</span>{" "}
              {hr.userName}
            </Typography>
            <Typography sx={{ mb: 1 }} color="text.secondary">
              <span style={{ fontWeight: "bold" }}> Hr Rating:</span>{" "}
              {user.hrRating === -1 ? "No Ratings Given" : user.hrRating}
            </Typography>
            <Typography sx={{ mb: 1 }} color="text.secondary">
              <span style={{ fontWeight: "bold" }}> Hr Comments:</span>{" "}
              {user.hrComments === "" ? "No Comments" : user.hrComments}
            </Typography>
            <Typography sx={{ mb: 1 }} color="text.secondary">
              <span style={{ fontWeight: "bold" }}>Tech Rating: </span>
              {user.techRating === -1 ? "No Ratings Given" : user.techRating}
            </Typography>
            <Typography color="text.secondary">
              <span style={{ fontWeight: "bold" }}>Tech Comments:</span>{" "}
              {user.techComments === " " ? "No Comments" : user.techComments}
            </Typography>
          </CardContent>
          <CardActions>
            <Button
